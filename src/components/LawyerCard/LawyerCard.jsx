...
export const LawyerCard = ({ lawyer, className }) => {
    return (
        <Link to={`/lawyer/${lawyer.id}`} className={`${styles.LawyerCard} ${className ? className : ""} px-5 flex justify-center items-center rounded-2xl shadow-lg border dark:bg-slate-400`}>
            <div>
                ...
                <h3 className=' dark:text-white text-start font-bold leading-5	pb-5 opacity-75 duration-500'><FormattedMessage id={"description"} /> : {lawyer.description || <FormattedMessage id='noDescription' />}</h3>
            </div>
        </Link>
    );
};
